...
bool TerrainShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"terrain.vs.hlsl", L"terrain.ps.hlsl");
	if (!result)
	{
		return false;
	}
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	...
	ID3D11ShaderResourceView* normalMap1, ID3D11ShaderResourceView* normalMap2, ID3D11ShaderResourceView* normalMap3, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture1, texture2, texture3, normalMap1, normalMap2, normalMap3, lightDirection, diffuseColor);
	if (!result)
	{
		return false;
	}
	...
	ID3D11ShaderResourceView* normalMap1, ID3D11ShaderResourceView* normalMap2, ID3D11ShaderResourceView* normalMap3, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	...
	deviceContext->PSSetShaderResources(3, 1, &normalMap1);
	deviceContext->PSSetShaderResources(4, 1, &normalMap2);
	deviceContext->PSSetShaderResources(5, 1, &normalMap3);
