...
bool TerrainClass::Initialize(ID3D11Device* device, char* setupFilename, bool initializeBuffers)
{
	bool result;
	m_initializeBuffers = initializeBuffers;
...
bool TerrainClass::LoadTerrainCells(ID3D11Device* device)
{
	...
	for (j = 0; j<cellRowCount; j++)
	{
		for (i = 0; i<cellRowCount; i++)
		{
			...
			result = m_TerrainCells[index].Initialize(device, m_model, i, j, cellHeight, cellWidth, m_terrainWidth);
			if (!result)
			{
				return false;
			}
...
bool TerrainClass::InitializeBuffers(ID3D11Device* device)
{
	...
	for (i = 0; i<m_vertexCount; i++)
	{
		...
		m_vertices[i].color = XMFLOAT3(m_model[i].r, m_model[i].g, m_model[i].b);
		indices[i] = i;
	}
...
void TerrainClass::ShutdownVertices()
{
	// Release the vertices model data.
	if (m_vertices)
	{
		delete[] m_vertices;
		m_vertices = 0;
...
bool TerrainClass::GetHeightAtPosition(float positionX, float positionZ, float& height)
{
	...
	meshMinX = 0.00f;
	meshMaxX = m_terrainWidth;

	meshMinZ = 0.00f;
	meshMaxZ = m_terrainWidth;
	...
	FindCell(positionX, positionZ, height);

	return true;
}

void TerrainClass::FindCell(float x, float z, float& height)
{
	...
	{
		m_TerrainCells[i].GetCellDimensions(xMax, yMax, zMax, xMin, yMin, zMin);

		// See if the x and z coordinate are in this cell, if not then return.
		...
		for (int j = 0; j<m_TerrainCells[i].GetVertexCount() / 3; j++)
		{
			index = j * 3;
			vertex1[0] = m_TerrainCells[i].m_vertexList[index].x;
			vertex1[1] = m_TerrainCells[i].m_vertexList[index].y;
			vertex1[2] = m_TerrainCells[i].m_vertexList[index].z;

			index++;
			vertex2[0] = m_TerrainCells[i].m_vertexList[index].x;
			vertex2[1] = m_TerrainCells[i].m_vertexList[index].y;
			vertex2[2] = m_TerrainCells[i].m_vertexList[index].z;

			index++;
			vertex3[0] = m_TerrainCells[i].m_vertexList[index].x;
			vertex3[1] = m_TerrainCells[i].m_vertexList[index].y;
			vertex3[2] = m_TerrainCells[i].m_vertexList[index].z;
